...
export const useStore = (props) => {
  ...
  useEffect(() => {
    ...
    // Cleanup on unmount
    return () => {
      try { messageListener.unsubscribe?.() } catch {}
      try { userListener.unsubscribe?.() } catch {}
      try { channelListener.unsubscribe?.() } catch {}
    }
  }, [])
...
export const addChannel = async (slug, user_id) => {
  try {
    ...
    console.log('error', error)
    return { data: null, error, status: undefined }
  }
}
...
// Role management RPC helpers
export const listUsersWithRoles = async () => {
  try {
    const { data, error } = await supabase.rpc('get_users_roles')
...
export const setUserRole = async (email, role, grant) => {
  try {
